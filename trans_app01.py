...
def find_face():
    ...
    # 顔検出の分類機を用意
    cascade_file = "haarcascade_frontalface_alt.xml"  # カスケードファイルを変数に格納
...
def paste_image(face_list):
    ...
    # resized_mask_img = cv2.resize(mask_img, dsize=(w, h))  # おたふく画像のりサイズ、顔認識したwとhの大きさに変更する
    ...
    # splitがアルファ(R[0]G[1]B[2]A[3]のA)を分けてるから背景を透明化できる
